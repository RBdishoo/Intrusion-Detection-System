...
#All feature columns are numeric, coerce errors, fill new NaNs
...
# Convert true labels to binary: 1 = Normal, 0 = Anomoly
...
try:
    ...
    # Classify predictions. If reconstruction_error > threshold, it's an anomaly (0); else it's normal (1)
...
try:
    ...
    # Manually calculate Isolation Forest threshold. The lower the scorem the more anomolous for IF's desicion funct. Improve precision by being more selective (increase threshold score) -> selecting a *lower* percentile of the original if_anomaly_scores. We are aiming to balance Recall with Precision.

    if_threshold = np.percentile(if_anomaly_scores, 1.5)
    ...
    #ROC AUC calculation for Isolation Forest:
